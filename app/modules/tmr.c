...
static union {
	uint64_t block;
	uint32_t part[2];
} rtc_time  DATA_IRAM_ATTR;
static sint32_t soft_watchdog  = -1;
static timer_struct_t alarm_timers[NUM_TMR] DATA_IRAM_ATTR;
static os_timer_t rtc_timer DATA_IRAM_ATTR;

extern void set_lua_timer(unsigned id);

#define alarm_timer_common(a) set_lua_timer(a)

void alarm_timer_common_cb(void* arg)
{
...
//	ets_timer_setfn(&tmr->os, alarm_timer_common, (void*)id);
	ets_timer_setfn(&tmr->os, (os_timer_func_t *)set_lua_timer, (void*)id);
	return 0;  
}
...
extern void run_sdk_tasks(void);
